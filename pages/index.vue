<template>
  <div class="home">
    ...
    <div class="container search">
      ...
      <button v-show="searchInput !== ''" class="button" @click="clearSearch">X</button>
    </div>
      ...
      <div v-if="searchInput !== ''" id="movies-grid" class="movies-grid">
        <div v-for="(movie, index) in searchedMovies"  :key="index" class="movie">
          ...
          <div class="info">
            ...
            <NuxtLink :to="`/movies/${movie.id}`" class="button button-light">Get more info</NuxtLink>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    async getMovies() {
      const data = axios.get(`https://api.themoviedb.org/3/movie/now_playing?api_key=5f2c91a3f464348846e742d417dfe044&language=en-US&page=1`);
    ...
    async searchMovie(){
      const data = await axios.get(`https://api.themoviedb.org/3/search/movie?api_key=5f2c91a3f464348846e742d417dfe044&language=en-US&page=1&query=${this.searchInput}`);
